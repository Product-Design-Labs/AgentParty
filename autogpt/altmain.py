...
def load_variables(config_file="config.yaml"):
    try:
        ...
        desired_outcomes = config.get("desired_outcomes")
        suggested_procedure = config.get("suggested_procedure")
    ...
    if not purpose:
        purpose = utils.clean_input("Enter the Purpose of Agent A: ")

    if not desired_outcomes:
        print("Enter Desired Outcomes for Agent A (type 'done' when finished):")
        while True:
    ...
    if not suggested_procedure:
        suggested_procedure = utils.clean_input("Enter the Suggested Procedure for achieving the desired outcomes: ")

    config = {"purpose": purpose, "desired_outcomes": desired_outcomes, "suggested_procedure": suggested_procedure}
    ...
    full_prompt = f"You are {ai_name}, Purpose: {purpose}\n{prompt_start}\n\nDESIRED OUTCOMES:\n\n"
...
def construct_prompt():
    ...
    if config.purpose:
        logger.typewriter_log(
            f"Welcome back! ",
            Fore.GREEN,
            f"Would you like me to continue with the last settings?",
            speak_text=True)
        should_continue = utils.clean_input(f"""Continue with the last settings?
Purpose:  {config.purpose}
Desired Outcomes: {config.desired_outcomes}
Suggested Procedure: {config.suggested_procedure}
Continue (y/n): """)
...
def prompt_user():
    ...
    logger.typewriter_log(
            ...
            Fore.GREEN,
            "Enter the Purpose, Desired Outcomes, and Suggested Procedure for Agent A.",
            speak_text=True)
    ...
    logger.typewriter_log(
        ...
        Fore.GREEN)
    purpose = utils.clean_input("Purpose: ")
    ...
    logger.typewriter_log(
        "Enter the Suggested Procedure for achieving the desired outcomes:",
        Fore.GREEN)
    suggested_procedure = utils.clean_input("Suggested Procedure: ")
    ...
    config = AIConfig(purpose, desired_outcomes, suggested_procedure)
    return config
